...
let uncoveredText = "Stroke hefted a small knife in her hand. It was heavy and stout, made to be more durable than sharp. She couldn’t remember if she had stolen this particular one or purchased it, but it was likely the former. Knives this good were not cheap."
...
let eragon = {id: 0, name: "Eragon by Cristopher Paoloni", text: eragonText};
...
app.post('/api/stories', (req, res) => {
  ...
  var title = "";
  if (req.body.name === "") {
    title = "User Story #" + (id - 2);
  }
  else {
    title = req.body.name;
  }
  let story = {id:id, name: title, text:req.body.text};
  stories.push(story);
  res.send(story);
});
...
app.put('/api/stories/:id', (req, res) => {
  ...
  let storiesMap = stories.map(story => { return story.id; });
  let index = storiesMap.indexOf(id);
